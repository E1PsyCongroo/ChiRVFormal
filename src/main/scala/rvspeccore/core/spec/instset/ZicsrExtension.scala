...
trait ZicsrExtensionInsts {
  ...
  val CSRRW  = Inst("CSRRW", "b????????????_?????_001_?????_1110011")
  val CSRRS  = Inst("CSRRS", "b????????????_?????_010_?????_1110011")
  val CSRRC  = Inst("CSRRC", "b????????????_?????_011_?????_1110011")
  val CSRRWI = Inst("CSRRWI", "b????????????_?????_101_?????_1110011")
  val CSRRSI = Inst("CSRRSI", "b????????????_?????_110_?????_1110011")
  val CSRRCI = Inst("CSRRCI", "b????????????_?????_111_?????_1110011")
...
trait ZicsrExtension
  ...
  def wen(addr: UInt, justRead: Bool = false.B): Bool = {
    ...
    val isIllegalMode  = now.privilege.mode < addr(9, 8)
